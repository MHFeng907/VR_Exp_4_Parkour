...
namespace Unity.UOS.Auth
{
    ...
    public class AuthTokenManager : GenericSingleton<AuthTokenManager>
    {
        ...
        private const string SdkVersion = "1.0.10";
        ...
        public static void Initialize(string uosAppID, string uosAppSecret)
        {
            ...
            try
            {
                if (Instance._appSettings.TryGetValue(uosAppID, out var appSetting))
                {
                    appSetting.UosAppID = uosAppID;
                    if (appSetting.UosAppSecret != uosAppSecret)
                    {
                        appSetting.UosAppSecret = uosAppSecret;
                        ...
                        appSetting.HttpClient = new HttpClient(new HttpClientOptions
                        {
                            ...
                            Authenticator = new UosNonceAuthenticator(uosAppID, uosAppSecret),
                            ExceptionHandler = ExceptionUtil.ExceptionHandler
                        });
                    }
                    appSetting.TokenPersistKey = $"passport:current_token:{uosAppID}";
                }
                else
                {
                    appSetting = new AppSetting
                    {
                        ...
                        TokenPersistKey = $"passport:current_token:{uosAppID}",
                    ...
                    Instance._appSettings[uosAppID] = appSetting;
                }
                Instance.CurrentUosAppID = uosAppID;
            }
            finally
            {
